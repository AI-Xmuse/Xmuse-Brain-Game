...
def get_osc_address_prefix():
    ...
    while input_active and not user_input_done:
        ...
        screen.blit(prompt_text, (50, height // 3))
        # 显示用户输入的内容
        input_surface = font.render(input_text, True, (255, 255, 255))
        screen.blit(input_surface, (50, height // 3 + 40))
        # 刷新屏幕并处理事件
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    ...
                    input_text += event.unicode  # 添加字符
    return int(input_text) if input_text.isdigit() else 9000  # 默认端口
...
def main():
    ...
    # 创建并启动OSC服务器线程
    osc_thread = Thread(target=osc_server_thread, args=(address_prefix, port_number))
