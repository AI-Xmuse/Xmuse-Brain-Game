...
def draw_ui():
    ...
    score_text = font.render(f"You: {player_score}  AI: {cpu_score}", True, colors["red"])
    win.blit(score_text, (40, 40))
...
def text_input_screen(title, default, validator=None):
    """通用文本输入界面，使用 TEXTINPUT 事件兼容中文输入法"""
    input_text = default
    active = True
    ...
    while active:
        ...
        # 提示
        prompt = font.render(title, True, colors["white"])
        win.blit(prompt, (50, height // 3 - 10))
        # 输入框背景
        box_rect = pygame.Rect(50, height // 3 + 30, width - 100, 40)
        pygame.draw.rect(win, (60, 60, 60), box_rect)
        pygame.draw.rect(win, colors["green"], box_rect, 2)
        # 输入文本 + 闪烁光标
        cursor_show = int(time.time() * 2) % 2 == 0
        display = input_text + ("|" if cursor_show else "")
        input_surf = font.render(display, True, colors["white"])
        win.blit(input_surf, (60, height // 3 + 38))
        # 操作提示
        hint = font.render("Enter=Confirm  Backspace=Delete  Esc=Use Default", True, colors["gray"])
